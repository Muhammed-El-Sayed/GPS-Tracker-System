...
float64 calculate_Distance_between_2_Coordinates(float64 lat1, float64 long1,float64 lat2,float64 long2) //latitude & longitude in degrees
{
    ...
    if(counter == 2)
    {
      if(buffer[i+1] != '.')
      {
        lat[l] = buffer[i+1]; //Degrees Minutes#include "GPS.h"

 float64 Return_Latitude_In_Degrees (void) //returns latitude in degrees (including converting minutes to degrees) & fill lat[] array with degrees & minutes
{
  unsigned char counter=0,l=0;
...
float64 Return_Longitude_In_Degrees (void) //returns longitude in degrees (including converting minutes to degrees) & fill lon[] array with degrees & minutes
{
  unsigned char counter=0,l=0;
  for(unsigned char i=0;i<100;++i)
  {
